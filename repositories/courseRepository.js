...
function getAllCourses(userId) {
  ...
  return new Promise((resolve, reject) => {
    knex_db
      .raw(sql, [userId])
      .then(() => {
        const sql2 = `SELECT id, title, level FROM courses`;
  ...
  return new Promise((resolve, reject) => {
    if (criteria == "name") {
      knex_db
        .raw(sql, [userId])
        .then(() => {
          const sql2 = `SELECT id, title, level FROM courses ORDER BY title`;
      ...
      knex_db
        .raw(sql, [userId])
        .then(() => {
          const sql2 = `SELECT id, title, level FROM courses WHERE level = ? ORDER BY title`;
